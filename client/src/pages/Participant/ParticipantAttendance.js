                        ...
                        {records.reverse().map((r) => (
                            <>
                                <VerticalTimelineElement
                                    ...
                                    contentStyle={r.attended ? {
                                        background: 'rgb(250, 250, 250)',
                                        color: 'black', borderTop: '2px solid rgb(0, 98, 72)'
                                    } : { background: 'rgb(250, 250, 250)', color: 'black', borderTop: 'solid 2px rgb(250, 0, 0)' }}
                                    contentArrowStyle={{ borderRight: '7px solid  rgb(250, 250, 250)' }}
                                    date={r.attended ? `Attended : ${r.session.start_date.split('T')[0]}`
                                        : `Missed :  ${r.session.start_date.split('T')[0]}`}
                                    iconStyle={r.attended ? { background: 'rgb(0, 98, 72)', color: 'white' }
                                        : { background: 'rgb(255, 30, 30)', color: 'white' }}
                                    animate
                                    icon={!r.attended ? <fa.FaCalendarCheck /> : <fa.FaCalendarDay />}
                                >
                                    <div className='d-flex'>
                                        <p>{(r.session.start_date.split('T')[1]).split('.')[0].split(':')[0]}:
                                            {(r.session.start_date.split('T')[1]).split('.')[0].split(':')[1]} </p>
                                        <p>{'-'}</p>
                                        <p> {(r.session.end_date.split('T')[1]).split('.')[0].split(':')[0]}:
                                            {(r.session.end_date.split('T')[1]).split('.')[0].split(':')[1]}</p>
                                    </div>
