...
export const loginOrg = (email, password) => async (dispatch) => {
  try {
    ...
    const loginResponse = await axios.post('/api/organisation/login', {
      email,
      password,
    });

    // If there were no errors then we can fetch the user profile
    const { data } = await axios.get(`/api/organisation/profile`);
...
export const register = (name, email, password) => async (dispatch) => {
  try {
    ...
    const registerResponse = await axios.post('/api/organisation', {
      username: name,
      email,
      password,
    });
